...
class UserService:
  ...
  def CancelTransaction(self):
    ...
    for i in range(len(user_tx_list)):
      if user_tx_list[i][1] == username[0] and user_tx_list[i][0] == tx_nr:
  ...
  def MineBlock(self):
    ...
    if block is None:
      ...
      start_dt = dt.datetime.strptime(last_time, '%H:%M:%S')
      end_dt = dt.datetime.strptime(time, '%H:%M:%S')
  ...
  def ConfirmBlock(self):
    ...
    if len(list_confirmed[i]) == 0:
        print(f"Welcome {username[0]}, you are the first user to cnfirm the block.")
    if len(list_confirmed[i]) == 1:
        print(f"Welcome {username[0]}, you are the second user to cnfirm the block.")
    if len(list_confirmed[i]) == 2:
        print(f"Welcome {username[0]}, you are the third user to cnfirm the block.")
    ...
    if len(new_list) == 3: 
      ...
      for i in range(len(hashed_before)): 
        if hashed_before[i][1] == recheck[i]:
          ...
          b = False
          print(f"Block number {hashed_before[i][0]} is tempered! {b}")
          time.sleep(0.2)
          
      
      mined_blocks = check_block_valid
      list_duration = []
      ...
      if block_to_confirm[1] == username[0]:
        ...
        #list_tx = self.MineBlock().list_stringify_tx
          ...
          for i in range(len(B)):
            if B[i][6] == min_duration:
              ...
              if Blc[4] == add_block[3]:
                ...
                print ("Success! Valid block is verified and added to the chain.")
        ...
        if len(confirming_status) == 3: 
          ...
          for user in user_list:
            ...
            tx_paid = 0
            for i in range(len(top_valid_tx)):
              another_user_list = top_valid_tx[i][1]
              print(another_user_list)
          ...
          self.userRepository.DeleteTempBlock()
          print(f"Temporary tables Tempblock and its hash were deleted.")
